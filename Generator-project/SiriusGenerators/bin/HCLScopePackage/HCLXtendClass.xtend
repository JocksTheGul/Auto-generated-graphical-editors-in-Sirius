...
public class HCLXtendClass {
	...
	{
		val gen = new HCLGenerator;
		
		//For checking the data from the metamodel
		val temp = HclScopePackage.eINSTANCE;
...
class HCLGenerator extends AbstractGenerator {
	...
	override doGenerate(Resource input, IFileSystemAccess2 fsa, IGeneratorContext context) {
		...
		val MetaModel = HclScopePackage.eINSTANCE;
		fsa.generateFile(MetaModel.name + ".odesign", MetaModel.compile)
	}
	
	public def compile(HclScopePackage mmPackage) '''
		...
		<ownedViewpoints name="«viewpoint»" modelFileExtension="«fileextension»">
		  «val mainclass = getMainClass(mmPackage)»
		  	    ...
		  	    <containerMappings name="«containerAnnotations.details.get("name")»"«IF editNameAnnotations !== null» labelDirectEdit="«getToolString(editNameAnnotations, viewpoint, diagram)»"«ENDIF» «IF label !== null»labelExpression="«label»" «ENDIF»semanticCandidatesExpression="«containerAnnotations.details.get("expression")»" domainClass="«mmPackage.name»::«containerAnnotations.details.get("domain")»"«IF presentation !== null» childrenPresentation="«presentation»"«ENDIF»>>
	...
	private def getMainClass(HclScopePackage mmPackage)
	{	//Function to get the main class
		...
		private def getPath(HclScopePackage mmPackage)
	{	//Function to get the main class
		//Can use the package directly to access the sirius annotation
		//as the sirius annotation is not connected to any element
		val contents = mmPackage.EAnnotations
	...
	private def getViewpoint(HclScopePackage mmPackage)
	{	
		val contents = mmPackage.EAnnotations
		for(i : 0 ..< contents.size())
		{	
		...
		private def getExtension(HclScopePackage mmPackage)
	{	
		val contents = mmPackage.EAnnotations
		for(i : 0 ..< contents.size())
		{	
	...
	private def getDiagram(HclScopePackage mmPackage)
	{	
		val contents = mmPackage.EAnnotations
		for(i : 0 ..< contents.size())
		{	
	...
	private def getNodes(HclScopePackage mmPackage)
	{
	...
	private def getAllNodes(HclScopePackage mmPackage)
	{
		//Get the contents of the package
		val contents = mmPackage.eAllContents.toList()
		//New list to contain the nodes
		...
		private def getAllContainers(HclScopePackage mmPackage)
	{
		//Get the contents of the package
		val contents = mmPackage.eAllContents.toList()
		//New list to contain the nodes
	...
	private def getContainers(HclScopePackage mmPackage)
	{
		//Get the contents of the package
		val contents = mmPackage.eAllContents.toList()
	...
	private def getContainerContents(HclScopePackage mmPackage, String name)
	{
		val contents = mmPackage.eAllContents.toList()
	...
	private def getContentsString(EAnnotationImpl annotation, HclScopePackage mmPackage)
	{
		//If it finds an element that has the node annotation add it to the list
	...
	private def getContainedContentsString(EAnnotationImpl annotation, HclScopePackage mmPackage)
	{
		//If it finds an element that has the node annotation add it to the list
		if(annotation.source == "sirius.borderednode")
		{
	...
	private def getBorderedNodeString(EAnnotationImpl objectAnnotation, HclScopePackage mmPackage)'''
	...
	private def getSubNodeString(EAnnotationImpl objectAnnotation, HclScopePackage mmPackage)'''
	...
	'''
	private def getSubContainerString(EAnnotationImpl objectAnnotation, HclScopePackage mmPackage)'''
	...
	private def getToolsAnnotation(HclScopePackage mmPackage, String tool, String name, EAnnotation objectAnnotation)
	{
		val contents = mmPackage.eAllContents.toList
		for(i : 0 ..< contents.size())
		{
	...
	private def getDoubleClickToolString(HclScopePackage mmPackage, EAnnotationImpl objectAnnotation)'''
	...
	private def getReconnectionTools(HclScopePackage mmPackage, String tool, String name)
	{
		val contents = mmPackage.eAllContents.toList
	...
	private def getUniversalTool(HclScopePackage mmPackage, String tool)
	{
		val contents = mmPackage.eAllContents.toList
		for(i : 0 ..< contents.size())
		{
	...
	private def getUniversalToolGroup(HclScopePackage mmPackage, String tool)
	{
		val contents = mmPackage.eAllContents.toList
	...
	private def getEdges(HclScopePackage mmPackage)
	{
		val contents = mmPackage.eAllContents.toList
	...
	private def getMappingString(HclScopePackage mmPackage, EAnnotationImpl objectAnnotation, String search)
	{
		var str = ""
	...
	private def getReusedContainers(HclScopePackage mmPackage)
	{
		val contents = mmPackage.eAllContents.toList
	...
	private def getContainerString(HclScopePackage mmPackage, EObject o, EAnnotationImpl objectAnnotation)
	{
			val contents = mmPackage.eAllContents.toList
			for(c : 0 ..< contents.size())
			{
		...
		private def getContainerStringSearch(HclScopePackage mmPackage, EAnnotationImpl objectAnnotation, String search)
	{
			val contents = mmPackage.eAllContents.toList
			for(c : 0 ..< contents.size())
			{
	...
	private def getAlternateEdge(HclScopePackage mmPackage, EAnnotationImpl objectAnnotation)
	{
		val contents = mmPackage.eAllContents.toList
		var str = ""
		for(c : 0 ..< contents.size())
	...
	private def getAlternateNode(HclScopePackage mmPackage, EAnnotationImpl objectAnnotation)
	{
		val contents = mmPackage.eAllContents.toList
		var str = ""
		for(c : 0 ..< contents.size())
	...
	<conditionnalStyles predicateExpression="«AlternateNode.details.get("condition")»">
		<style xsi:type="style:«AlternateNode.details.get("style")»" labelSize="12" «IF label !== null»labelExpression="«AlternateNode.details.get("label")»" «ELSE»labelExpression="«SourceNode.details.get("label")»" «ENDIF»labelPosition="node" resizeKind="NSEW">
			...
			<labelColor xsi:type="description:SystemColor" href="environment:/viewpoint#//@systemColors/@entries[name='black']"/>
        	«IF alternateColor !== null»
        		«getNodeColorString(AlternateNode)»
        	«ELSE»
        		«getNodeColorString(SourceNode)»
        	«ENDIF»
		</style>
	</conditionnalStyles>
	'''		

	private def getDecorations(HclScopePackage mmPackage)
	{
		val contents = mmPackage.eAllContents.toList
		var arr = newArrayList
		for(c : 0 ..< contents.size())
	...
	private def getDecorationString(ArrayList<EAnnotationImpl> arr, HclScopePackage mmPackage) '''
